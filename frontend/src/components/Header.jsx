...
const Header = () => {
  ...
  return (
    <header className="w-full bg-[#f7f4ef] pt-6 pb-3 relative z-30">
      ...
      {/* Nav */}
      <nav className="hidden md:flex justify-center items-center gap-7 mt-7 pb-2 border-b border-[#e3dcd1]">
        {navLinks.map((link) => (
          <a
            key={link.label}
            href={link.href}
            className="text-[#3a3a3a] text-[11px] tracking-[0.22em] uppercase hover:text-[#a08f7d] transition-colors duration-300 whitespace-nowrap"
          >
            {link.label}
          </a>
        ))}
        ...
        <nav className="md:hidden flex flex-col items-center gap-4 mt-6 pb-5 border-b border-[#e3dcd1] bg-[#f7f4ef]">
          {navLinks.map((link) => (
            <a
              ...
              className="text-[#3a3a3a] text-[11px] tracking-[0.22em] uppercase"
            >
              {link.label}
            </a>
          ))}
